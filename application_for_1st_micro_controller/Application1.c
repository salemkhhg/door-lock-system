/* ---------------------------------------------------------------------------------------------------
 [FILE NAME]: <Applicaition1_for_1st_micrio-controller.c>
...
int main(void)
{
	...
	uint8 flag = 0;
	uint8 val1; /*to read the value from EEPROM in it*/
	...
	val1 = eepromReadByte(0x0D); //Read a value (0x05) from address (0x0D) in Internal EEPROM to check if the first time
	if (val1 != 0x05)
	{
		flag = 0;
	}
